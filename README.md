...
- **물리 그룹(혼잡도 합산)**: 방향성 때문에 한 장소를 두 노드로 분리(입구/출구, 진입/진출 게이트,
  승차/하차)해도, 노드 속성의 **"물리 그룹"** 에 같은 이름을 주면 혼잡도가 **물리적으로 하나인
  장소로 합산**되어 산출된다(대시보드·히트맵·CSV·X.npz 모두 그룹 단위). 기본 예제는 입구·출구 →
  "출입구", 진입·진출 게이트 → "게이트", 승차·하차 → "승강장1" 로 묶여 6노드가 3개 물리 장소로 집계된다.
