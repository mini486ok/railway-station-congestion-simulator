...
- **노드 단위 + 물리 그룹 단위 동시 출력**: CLI(`cli.py`)는 `out/`에 **루트 평면 세트(설정 단위, 하위호환)**
  와 함께 **`node/`(노드별)·`group/`(물리 그룹별)** 두 GNN 구성 세트를 모두 만든다. 웹앱은 ExportPanel 의
...
## 검증·고도화 (전문가 다관점 × 2회 리뷰 반영)

서로 다른 관점의 에이전트(외부 LLM 코드리뷰 = Codex 2종, 시뮬레이션·철도 도메인 현실성, ML/STGCN 데이터,
UI/UX, 코드 품질)가 매 보완마다 2회에 걸쳐 비판적 리뷰를 수행했고, 타당하고 위험이 낮은 항목을 선별 반영했다.

**3차 보완 반영(요지)**: 노드/링크 **복사·붙여넣기**(Ctrl+C/V), **노드 단위+물리 그룹 단위 동시 출력**
(CLI 루트 호환 + `node/`·`group/` + 웹 **전체 번들 ZIP**), 시간 용어 **‘스텝/초’ 통일**, 사용법 문서 대폭 확장
(분포·동적 체류확률·CTM 개념·FAQ·STGCN 학습 가이드), **신규 복잡 템플릿**(동·서 대합실 분리형, 초대형 복합환승역
=54노드 27그룹·3노선·지하3층), **현실적 기본값 재조정**(07:00 램프업, 설비군 처리율, 면적·재차·배차 현실화로
영구 포화 노드 0개 달성), npz `value_scale`/`requested_output_level` 메타 + 다중 run 스키마 가드, Poisson 관측노이즈.
...
- **시간대별 OD/분기비(동적 가중치)**: 현재 링크 가중치는 정적이라 07~08시 도심방향 편중, 피크 후 환승·진출
  비중 변화 같은 시간대별 OD를 반영하지 못한다(도메인 리뷰 지적). 시간대 프로파일 기반 가중치 변조 필요.
- **양방향 동적 용량 공유**: 같은 물리 그룹의 상·하행 노드가 같은 폭/계단을 공유하지만, 현재는 방향 노드별로
  용량·면적이 독립이라 "한 방향이 붐비면 반대 방향도 좁아지는" 상호 간섭이 약하다. 그룹 합산 수요 기반 감쇠 필요.
- **열차 지연(delay_std) 활성화**: 하차/승차를 분리 노드로 모델링해 독립 지연 시 비동기화되므로 현재 `delay_std=0`
  으로 동기화한다. 같은 열차 이벤트 id를 공유하는 구조로 바꾸면 노선별 지연 변동(10~30초)을 안전하게 줄 수 있다.
...
- **예제 템플릿(8종)**: 상단 "📁 템플릿" 에서 내장 예제(양방향 표준역 / 단순 통로형 / 엘리베이터 환승 /
  2개 승강장 분기 / 환승역(2개 노선) / 심층 역사(병렬 수직동선) / **동·서 대합실 분리형** /
  **초대형 복합환승역(10출입구·3노선·지하3층, 54노드 27그룹)**)를 불러오거나,
  **현재 구성을 내 템플릿으로 저장·불러오기·삭제**(브라우저 localStorage). 모든 내장 예제는 양방향 2노드·물리 그룹 구조.
- **노드/링크 복사·붙여넣기**: 그래프에서 노드를 선택(빈 곳 드래그로 다중 선택)하고 **Ctrl+C → Ctrl+V**
  (또는 툴바 "복사"/"붙여넣기")로 그 사이 내부 링크까지 함께 복제한다(새 id·새 물리 그룹·위치 오프셋).
