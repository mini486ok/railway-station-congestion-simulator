...
| 파일 | 내용 |
|---|---|
| `X.npz` | `X[T,N,F]` 특징 텐서 + `adjacency[N,N]` + `edge_index[2,M]` + `edge_attr` + 정규화 통계 |
| `nodes.csv` | 노드 메타(id, name, kind, area) |
...
- **예제 템플릿**: 상단 "📁 템플릿" 에서 내장 예제(단방향 역사 / 단순 통로형 / 엘리베이터 환승 /
  2개 승강장 분기)를 불러오거나, **현재 구성을 내 템플릿으로 저장·불러오기·삭제**(브라우저 localStorage).
- **물리 그룹(혼잡도 합산)**: 방향성 때문에 한 장소를 두 노드로 분리(입구/출구, 진입/진출 게이트)해도,
  노드 속성의 **"물리 그룹"** 에 같은 이름을 주면 혼잡도가 **물리적으로 하나인 장소로 합산**되어
  산출된다(대시보드·히트맵·CSV·X.npz 모두 그룹 단위). 기본 예제는 입구·출구 → "출입구",
  진입·진출 게이트 → "게이트" 로 묶여 5노드가 3개 물리 장소로 집계된다.
...
- 자체발생: 출입구(Poisson/Normal/Constant + 러시아워 프로파일), 승강장(열차 하차 버스트).
